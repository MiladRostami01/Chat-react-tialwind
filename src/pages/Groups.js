    ...
    <div className={`w-full grid gap-4 row-auto md:overflow-y-auto md:h-[436px] h-auto ${layoutStyle}`}>
      <Card>
          <div className='h-[56px] w-[56px] flex justify-center items-center cursor-pointer bg-veryDark rounded-full'>
            ...
            <path d="M16 6.66666V25.3333" stroke="#808FBE" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
            <path d="M6.66666 16H25.3333" stroke="#808FBE" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
          </div>
          <div className='text-textColor text-[18px] mt-2 font-medium ml-4'>Create Group</div>
      </Card>
      {GroupsData.map((group) => {
        return( 
        <Card key={group.id}> 
          <div><img src={group.image} alt={group.image} /></div>

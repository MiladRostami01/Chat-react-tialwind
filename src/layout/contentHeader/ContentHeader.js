...
function ContentHeader({ showSidebar, title }) {
  ...
  return (
    <header className="title-header">
      ...
      <div className="flex items-center justify-between text-textColor">
        <img src={arrow} alt="" /> <span className="text-3xl ml-[5px] ">{title}</span>
      </div>
      ...
      <div className="hidden lg:flex items-center justify-between border-extraDark border-solid border-[3px] rounded-[10px] overflow-hidden">
        <div className={gridStyle} onClick={() => setLayoutMode("grid")}>
          <img src={four} alt="" className="w-full h-full" />
        </div>
        <div className={listStyle} onClick={() => setLayoutMode("list")}>
